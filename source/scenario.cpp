...
hitable *two_perlin_spheres() {
    // texture *pertext = new noise_texture(3.0);
    int nx, ny, nn;
    std::cerr << "Loading texture from earth.jpg" << std::endl;
    std::cerr << "cwd: " << std::filesystem::current_path() << std::endl;
    unsigned char *tex_data = stbi_load("images/earth.jpg", &nx, &ny, &nn, 0);
    if (!tex_data) {
        std::cerr << "Error loading texture: " << stbi_failure_reason() << std::endl;
        return nullptr;
    }
    texture *pertext = new image_texture(tex_data, nx, ny);
...
hitable *random_scene() {
    ...
    hitable* BVH =  new bvh_node(list, i, 0, 1.0);
    // hitable* h_list = new hitable_list(list, i);
    return BVH;
}
